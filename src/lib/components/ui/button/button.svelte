	...
	export const buttonVariants = tv({
		...
		variants: {
			variant: {
				...
				outline: "border-dashed border-2 border-border bg-background font-semibold hover:bg-muted text-primary hover:text-primary/80 dark:bg-input/30 dark:border-input dark:hover:bg-input/50 aria-expanded:bg-muted aria-expanded:text-primary/80",
			...
			size: {
				default: "h-12 gap-1.5 px-3 has-data-[icon=inline-end]:pr-2 has-data-[icon=inline-start]:pl-2",
				...
				lg: "h-14 gap-1.5 px-3.5 has-data-[icon=inline-end]:pr-2.5 has-data-[icon=inline-start]:pl-2.5",
				icon: "size-12",
				"icon-xs": "size-8 rounded-[min(var(--radius-md),10px)] in-data-[slot=button-group]:rounded-lg [&_svg:not([class*='size-'])]:size-3",
				"icon-sm": "size-10 rounded-[min(var(--radius-md),12px)] in-data-[slot=button-group]:rounded-lg",
				"icon-lg": "size-14",
			},
		},
		defaultVariants: {
